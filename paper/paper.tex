...
Setup function to run learners in parallel using distributed computing.
\begin{lstlisting}
...
We can now benchmark the performance of the different machine learners using a sensor dataset by creating a dictionary of workers from Caret, ScikitLearn, and Julia libraries and call the `parallelmode` function:

\begin{lstlisting}
learners=Dict(
  :jvote_ens=>jvote_ens,:jstack_ens=>jstack_ens,
  :jbest_ens=>jbest_ens,:jrf => jrf,:jada=>jada,
  :jsuper_ens=>jsuper_ens,#:crt_rpart=>caret_rpart,
  :crt_svmlinear=>caret_svmlinear,
  :crt_treebag=>caret_treebag,#:crt_rf=>caret_rf, 
  :skl_knn=>sk_knn,:skl_gb=>sk_gb,
  :skl_extratree=>sk_extratree,
  :sk_rf => sk_rf
);
