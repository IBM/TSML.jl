...
function test_typesoutliernicer()
  ...
  @test round(sum(regulardf[1,3:20])) < 0.0 
  m=fit(regpipeline)
  ...
  @test round(sum(monodf[1,3:20])) < 0.0
  m=fit(monopipeline)
  monodf=transform(m)
  @test round(sum(monodf[1,3:20])) < 0.0
  ...
  @test round(sum(dailymonodf[1,3:20])) < 0.0 
