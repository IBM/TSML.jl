...
function test_crossvalidator()
  ...
  rf = RandomForest()
  @test crossvalidate(rf,X,Y,acc).mean ≈ 94.0
  Random.seed!(123)
  ppl1 = Pipeline(Dict(:transformers=>[RandomForest()]))
  @test crossvalidate(ppl1,X,Y,acc).mean ≈ 94.0
  Random.seed!(123)
  ohe = OneHotEncoder()
  stdsc= StandardScaler()
  ppl2 = Pipeline(Dict(:transformers=>[ohe,stdsc,RandomForest()]))
  @test crossvalidate(ppl2,X,Y,acc).mean ≈ 94.66666667
  Random.seed!(123)
  ...
  @test crossvalidate(ppl5,X,Y,acc).mean ≈ 77.333333
end
